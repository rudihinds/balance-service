    ...
    stage('Deploy to Kubernetes') {
      withCredentials([
        ...
        container(name: 'kubectl', shell: '/bin/sh') {
          ...
          """
          sh "yq w -i kubernetes/app-deployment.yml 'spec.template.spec.containers[0].image' ${app_image_name}"
          sh "yq w -i kubernetes/worker-deployment.yml 'spec.template.spec.containers[0].image' ${worker_image_name}"
